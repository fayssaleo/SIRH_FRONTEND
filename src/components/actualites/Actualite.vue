<template>
  <b-row>
    <b-col class="pt-0 mt-0 mb-0 pb-0" cols="12">
      <img
        v-if="image != null"
        class="newsPic"
        :src="getActualiteImagesFolder + '/' + image"
        alt=""
        :style="{
          height: height,
        }"
    ...
    <b-col
      class="newTitle pt-0 mt-1 mb-0 pb-0"
      cols="12"
      :style="{ font: font }"
      >{{ titre.toUpperCase() }}
    </b-col>
    <b-col
      class="created_at pt-2 mt-0 mb-0 pb-0"
      cols="12"
      :style="{ color: color }"
      ><span class="pr-2"
        ><svg
      ...
      ><span class="newDate">{{ created_at }}</span></b-col
    >
  </b-row>
</template>
<script>
...
export default {
  props: ["titre", "image", "created_at", "height", "font", "color"],
  computed: {
