...
export default {
  ...
  methods: {
    ...mapActions([
      "addActualites_categorieAction",
      "editActualites_categorieAction",
      "deleteActualites_categorieAction",
    ]),
    closeDialog() {
      this.dialog = false;
    },
    ...
    categorieSelectChange() {
      let categorieToEdit = this.getActualites_categories.filter(
        (c) => c.id == this.categorie.id
      )[0];
      this.infoFieldsDisabled = true;
      this.infoFieldsDisabled = true;
      this.categorie.libelle = categorieToEdit.libelle;
      this.$refs.nom.focus();
    ...
    categorie_submit() {
      ...
      if (this.isValid) {
        if (this.modeAdd) {
          this.addActualites_categorieAction(this.categorie).then(() => {
            this.infoFieldsDisabled = true;
            this.categorie = {
              id: null,
              libelle: "",
          ...
          this.editActualites_categorieAction(this.categorie).then(() => {
            this.infoFieldsDisabled = true;
            this.categorie = {
              id: null,
              libelle: "",
    ...
    deleteCategorie() {
      this.deleteActualites_categorieAction({
  ...
  props: ["value"],
  computed: {
    ...mapGetters(["getActualites_categories"]),
    dialog: {
      get() {
        return this.value;
      },
